...
#ifdef _WIN64
    DoOutputDebugString("CAPE initialised: 64-bit DumpOnAPI package loaded at 0x%p, process image base 0x%p\n", g_our_dll_base, GetModuleHandle(NULL));
#else
    DoOutputDebugString("CAPE initialised: 32-bit DumpOnAPI package loaded at 0x%p, process image base 0x%p\n", g_our_dll_base, GetModuleHandle(NULL));
#endif
