...
HOOKDEF(LPVOID, WINAPI, VirtualProtectEx,
	_In_  HANDLE hProcess,
	_In_  LPVOID lpAddress,
	_In_  SIZE_T dwSize,
	_In_  DWORD  flNewProtect,
	_Out_ PDWORD lpflOldProtect)
{
	LPVOID ret = Old_VirtualProtectEx(hProcess, lpAddress, dwSize, flNewProtect, lpflOldProtect);
	if (lpflOldProtect && *lpflOldProtect)
		LOQ_nonnull("misc", "ppiii", "ProcessHandle", hProcess, "Address", lpAddress, "Size", dwSize, "NewProtect", flNewProtect, "OldProtect", *lpflOldProtect);
	else
		LOQ_nonnull("misc", "ppiii", "ProcessHandle", hProcess, "Address", lpAddress, "Size", dwSize, "NewProtect", flNewProtect, "OldProtect", 0);
	return ret;
}
